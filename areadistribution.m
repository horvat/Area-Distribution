...
dumpfreq = 10; %Frequency of saving
dt = 1; %Time interval
...
[A0,T,D,H,g,f,shiftra,shiftri,Yg,lambda,Spectrum,target] = load_simp_IC(nbins);
...
for t = 2:nt+1
    ...
    Redist = 100*redist_fsd(A,epsdot,f,D,shiftra,shiftri);
   % Redist = 0*D;
   ...
   % Swell = 0*D;
    ...
    Save = Save + [Advect; Melt; Redist; Swell];
    
    %% Add it all up
    DA  = Advect  + Melt + Redist + Swell; 
    
    Anew = A + dt*DA; 
  %  Anew(1) = 1 + Anew(1) - sum(Anew);
...
plotFSD(D,A,A0,A1,stats,Save,Timer);
